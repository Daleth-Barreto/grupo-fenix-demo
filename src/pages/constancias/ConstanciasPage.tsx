...
export default function ConstanciasPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f9f9ff] pb-24">
      <TopBar title="Constancias" />

      <main className="px-5 pt-5 pb-10 max-w-2xl mx-auto">
        <section className="mb-7 text-center">
          <h1 className="text-[28px] font-bold text-[#0a192f]">Constancias</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {constancias.map((c) => (
              ...
              >
                ...
                {/* Preview */}
                <div className="w-full h-32 rounded-xl bg-[#0a192f] flex items-center justify-center relative z-10 overflow-hidden">
                  {c.preview_url ? (
                ...
                <div className="relative z-10">
                  <h3 className="text-[15px] font-semibold text-[#0a192f] leading-snug">{c.title}</h3>
                  <p className="text-xs text-[#44474d] mt-1">{c.issued_at}</p>
                </div>

                {/* Acciones */}
                <div className="flex gap-3 relative z-10">
                  <button className="flex-1 border border-[#0a192f] text-[#0a192f] text-sm font-semibold py-3 rounded-xl hover:bg-[#f0f3ff] transition-colors active:scale-[0.98]">
                    Descargar PDF
                  </button>
                  <button className="flex-1 bg-[#fd761a] text-white text-sm font-semibold py-3 rounded-xl hover:bg-orange-500 transition-colors shadow-sm active:scale-[0.98]">
                    Ver
                  </button>
                </div>
              </article>
